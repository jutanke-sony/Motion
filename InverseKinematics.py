    ...
    def __call__(self):
        ...
        for i in range(self.iterations):
            ...
            for j in AnimationStructure.joints(self.animation.parents):
                ...
                anim_positions = anim_transforms[
                    :, :, :3, 3
                ]  # look at the translation vector inside a rotation matrix
                ...
                jdirs = (
                    anim_positions[:, c] - anim_positions[:, np.newaxis, j]
                )  #  limb vectors given by animation
                ddirs = (
                    self.positions[:, c] - self.positions[:, np.newaxis, j]
                )  #  limb vectors given by input positions (target)

                if (
                    len(c) > 1 and (jdirs == 0).all() and (ddirs == 0).all()
                ):  # there was an expansion of high degree vertices (expand_topology())
                ...
                if rotations.shape[1] == 1:
                    ...
                    averages = Quaternions.exp(
                        rotations[:, dirs_positive].log().mean(axis=-2)
                    )
            ...
            if self.threshold > 0:
                ...
                if not self.silent:
                    print(
                        "[BasicInverseKinematics] Iteration %i Error: %f"
                        % (i + 1, error)
                    )
...
def animation_from_positions(
    positions, parents, offsets=None, iterations=1, threshold=0.001
):
    ...
    if offsets is None:
        ...
        normed_offsets_no_root = np.divide(
            orig_offsets_no_root,
            bone_lens,
            out=np.zeros_like(orig_offsets_no_root),
            where=bone_lens != 0,
        )
    ...
    anim, sorted_order, parents = Animation.animation_from_offsets(
        offsets, parents, positions.shape
    )
    positions = positions[:, sorted_order]
    sorted_root_idx = np.where(sorted_order == root_idx)[0][0]

    anim.positions[:, sorted_root_idx] = positions[
        :, 0
    ]  # keep root positions. important for IK

    # apply IK
    ik = BasicInverseKinematics(
        anim, positions, silent=False, iterations=iterations, threshold=threshold
    )
    new_anim = ik()
