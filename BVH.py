...
def load(
    ...
    order=None,
    world=True,
    keep_end_site_no_matter_what=True,
):
    """
    ...
    for line in f:
        ...
        if offmatch:
            ...
            if not keep_end_site_no_matter_what and end_site and all(offsets[active] == 0):
                ...
                end_site_is_joint = False
                # print("ignore end site with zero offset->", debug_name)
            continue
    ...
    if np.isclose(offsets[1], 0).all():
        if len(parents[parents == 1]) == 0:  # redundant joint #1, just remove
            ...
            len(parents[parents == 0]) == 1
        ):  # remove root joint by composing rots, adding pos & offs
            offsets[1] = offsets[0]
            offsets = offsets[1:]
